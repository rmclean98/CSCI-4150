...
from ImageNegativeTransformation import ImageNegativeTransformation

class ImageNegativeUI(QWidget):

    # Creates UI elements
    def __init__(self, parent=None):
        super(ImageNegativeUI, self).__init__(parent)
    ...
    def startWindow(self):
        PageName=QLabel()
        PageName.setText("Image Negative Transformation")
    ...
    def processTransform(self):
        if len(self.txtFile.text())>0:
            if os.path.isfile(self.txtFile.text()):
                ImageNegativeTransformation.ProcessTransformation(self.txtFile.text())
        else:
