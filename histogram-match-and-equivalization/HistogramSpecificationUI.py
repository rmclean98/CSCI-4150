...
from ProcessHistogramSpecification import ProcessHistogramSpecification


class HistogramSpecificationUI(QWidget):

    # Creates UI elements
    def __init__(self, parent=None):
        super(HistogramSpecificationUI, self).__init__(parent)
        self.txtFile = QLineEdit()
        self.file_button = QPushButton('Select Original Image File to Transform')
        self.txtFile2 = QLineEdit()
        self.file_button2 = QPushButton('Select Specified Image File')
    ...
    def startWindow(self):
        PageName=QLabel()
        PageName.setText("Histogram Matching/Specification")
            ...
            if os.path.isfile(self.txtFile.text()):
                if len(self.txtFile2.text()) > 0:
                    if os.path.isfile(self.txtFile2.text()):
                        ProcessHistogramSpecification.ProcessTransformation(self.txtFile.text(), self.txtFile2.text())
