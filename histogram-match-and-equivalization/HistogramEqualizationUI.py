...
from ProcessHistogramEqualization import ProcessHistogramEqualization

class HistogramEqualizationUI(QWidget):

    # Creates UI elements
    def __init__(self, parent=None):
        super(HistogramEqualizationUI, self).__init__(parent)
    ...
    def startWindow(self):
        PageName=QLabel()
        PageName.setText("Histogram Equalization")
    ...
    def processTransform(self):
        if len(self.txtFile.text())>0:
            if os.path.isfile(self.txtFile.text()):
                ProcessHistogramEqualization.ProcessTransformation(self.txtFile.text())
        else:
